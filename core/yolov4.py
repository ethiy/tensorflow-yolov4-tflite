...
def decode(conv_output, output_height, output_width, NUM_CLASS, STRIDES, ANCHORS, i, XYSCALE=[1,1,1], FRAMEWORK='tf'):
    if FRAMEWORK == 'trt':
        return decode_trt(conv_output, output_height, output_width, NUM_CLASS, STRIDES, ANCHORS, i=i, XYSCALE=XYSCALE)
    elif FRAMEWORK == 'tflite':
        return decode_tflite(conv_output, output_height, output_width, NUM_CLASS, STRIDES, ANCHORS, i=i, XYSCALE=XYSCALE)
    else:
        return decode_tf(conv_output, output_height, output_width, NUM_CLASS, STRIDES, ANCHORS, i=i, XYSCALE=XYSCALE)

def decode_train(conv_output, output_height, output_width, NUM_CLASS, STRIDES, ANCHORS, i=0, XYSCALE=[1, 1, 1]):
    conv_output = tf.reshape(conv_output,
                             (tf.shape(conv_output)[0], output_height, output_width, 3, 5 + NUM_CLASS))

    conv_raw_dxdy, conv_raw_dwdh, conv_raw_conf, conv_raw_prob = tf.split(conv_output, (2, 2, 1, NUM_CLASS),
                                                                          axis=-1)

    xy_grid = tf.meshgrid(tf.range(output_width), tf.range(output_height))
...
def decode_tf(conv_output, output_height, output_width, NUM_CLASS, STRIDES, ANCHORS, i=0, XYSCALE=[1, 1, 1]):
    batch_size = tf.shape(conv_output)[0]
    conv_output = tf.reshape(conv_output,
                             (batch_size, output_height, output_width, 3, 5 + NUM_CLASS))
...
def decode_tflite(conv_output, output_height, output_width, NUM_CLASS, STRIDES, ANCHORS, i=0, XYSCALE=[1,1,1]):
...
def decode_trt(conv_output, output_height, output_width, NUM_CLASS, STRIDES, ANCHORS, i=0, XYSCALE=[1,1,1]):
    batch_size = tf.shape(conv_output)[0]
    conv_output = tf.reshape(conv_output, (batch_size, output_height, output_width, 3, 5 + NUM_CLASS))
    ...
    pred_xy = tf.reshape(pred_xy, (batch_size, output_height, output_width, 3, 2))
    pred_wh = (tf.exp(conv_raw_dwdh) * ANCHORS[i])
    pred_xywh = tf.concat([pred_xy, pred_wh], axis=-1)
...
def filter_boxes(box_xywh, scores, score_threshold=0.4, input_shape = tf.constant([640,480])):
...
def compute_loss(pred, conv, label, bboxes, STRIDES, NUM_CLASS, IOU_LOSS_THRESH, i=0):
    ...
    input_height = STRIDES[i] * output_height
    input_width = STRIDES[i] * output_width
    conv = tf.reshape(conv, (batch_size, output_height, output_width, 3, 5 + NUM_CLASS))
    ...
    bbox_loss_scale = 2.0 - 1.0 * label_xywh[:, :, :, :, 2:3] * label_xywh[:, :, :, :, 3:4] / (input_height * input_width)
